...
# ----------------------------------------------------------------------------
# New feature functions: search, flashcards/quiz, translation
# ----------------------------------------------------------------------------
    ...
    if uploaded_files:
        ...
        with test_tab:
            ...
            if test_choice == "Flashcards":
                ...
                cards = st.session_state.test_cards.get(selected_doc, [])
                ...
                if selected_doc not in st.session_state.test_quiz:
                    with st.spinner("Generating quiz..."):
                        st.session_state.test_quiz[selected_doc] = generate_quiz(pages)

                quiz = st.session_state.test_quiz.get(selected_doc, [])
                ...
                if not quiz:
                    ...
                    for i, q in enumerate(quiz):
                        ...
                        st.radio(
                            "Choose one",
                            q["options"],
                            key=f"quiz_answer_{selected_doc}_{i}",
                            label_visibility="collapsed",
                        )
                        st.write("")
                    ...
                    if st.session_state.quiz_submitted:
                        correct_count = 0
                        for i, q in enumerate(quiz):
                            selected = st.session_state.get(f"quiz_answer_{selected_doc}_{i}")
